...
export default function LoginClient() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-6 sm:px-6 lg:px-10 py-4">
      <div className="mb-6">
        <h1 className="text-2xl sm:text-3xl font-bold">Login to your Account</h1>
        <p className="text-gray-600 dark:text-gray-300">Welcome back! Select the below login methods.</p>
      </div>
      ...
      <div className=" rounded-2xl p-10 sm:p-8 shadow-sm shadow-indigo-500">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
          ...
          <form onSubmit={onSubmit} className="space-y-4 order-2 md:order-1">
            {/* Email */}
            <div>
              <label className="block text-sm font-medium mb-2">Email ID / Username</label>
              <input
                type="email"
                value={email}
            ...
            <p className="text-sm text-gray-600">
              ...
              <Link href={`/register?next=${encodeURIComponent(next)}`} className="text-[#7C3AED] font-medium">
                Register
              </Link>
            </p>
          </form>
          ...
          <div className="order-1 md:order-2 flex justify-center">
            <Image src="/images/loginill.png" alt="Login illustration" width={500} height={500} className="max-w-full h-auto" priority />
          </div>
        </div>
      </div>
    </div>
